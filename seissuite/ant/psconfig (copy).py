...
# TRY AND FIX THIS SOLUTION SOON!
...
def create_config_list(basedir=os.path.join(os.getcwd(), 'configs'), 
    ...
    This function will return a list of only the files to be processed
    """
...
if FOLDER == "DEFAULT":
    ...
    TOMO_DIR = "{}/OUTPUT/TOMO".format(fold)
    DEPTHMODELS_DIR = "{}/OUTPUT/DEPTH".format(fold)
    ...
    TOMO_DIR = "{}/OUTPUT/TOMO".format(FOLDER)
    DEPTHMODELS_DIR = "{}/OUTPUT/DEPTH".format(FOLDER)  
...
FIRSTDAY = dt.datetime.strptime(FIRSTDAY, '%d/%m/%Y')
LASTDAY = config.get('cross-correlation', 'LASTDAY')
LASTDAY = dt.datetime.strptime(LASTDAY, '%d/%m/%Y')
...
XCORR_INTERVAL = config.getfloat('cross-correlation', 'XCORR_INTERVAL') 
...
try:
    ...
    CROSSCORR_TMAX = shift(XCORR_INTERVAL * 60.0)
