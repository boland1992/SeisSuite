...
for i in range(0, 100):   
  ...
  #      from seissuite.database import response_database
    ...
    # only process the first day's data
    
    process_number = 1440.0 / XCORR_INTERVAL
    
    if process_number < 1:
        raise Exception("Cross-correlation interval is greater than one day. \
Therefore new workflow method will not work")
    else: 
        process_number = int(process_number)
        
    print dates[:process_number] 
    quit()
