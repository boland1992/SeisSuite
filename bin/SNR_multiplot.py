...
for pair in stat_list:        
    ...
    for PICKLE_PATH in pickle_list:    
        ...
        outfile = OUT_SNR
        info_array = np.asarray(xc[s1][s2].SNR_lin)
    
        if len(info_array) > 0:
            SNRarray, timearray = info_array[:,0], info_array[:,1]      
            x, y = timearray,  SNRarray
            s = '{s1}-{s2}: 100 random stack combinations of SNR vs. time.'
            ...
            file_name = 'Random_Combination_{}-{}-SNR.png'.format(s1, s2)
