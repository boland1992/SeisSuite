...
#shape_path = '/storage/ANT/PROGRAMS/SHAPE/coastlines-split-4326/lines.shp'
#------------------------------------------------------------------------------
# CLASSES
#------------------------------------------------------------------------------
...
class Coordinates:
    ...
    def decluster(self,  inputs=None, degree_dist=1.):
        """
    ...
    # Select only 1 random point from each cluster to keep. Remove all others!          
        ...
        return np.append(inputs, cluster_keep, axis=0)
...
class Density:
    ...
    def plot_field(self, grad=False, SHAPE=None, swell=0.05, nodes=None):
        
        lonmin, lonmax, latmin, latmax = self.plot_lims()
        
        lonmin, lonmax, latmin, latmax = -130., -70., 24., 50. 
        
        xscale = 15
        yscale = (latmax-latmin) / (lonmax-lonmin) * xscale
        
        print yscale
        
        fig = plt.figure(figsize=(xscale,yscale), dpi=100)
        ...
        if SHAPE is not None:
            ...
            print nodes
            print type(nodes)
            plt.scatter(nodes[:,0], nodes[:,1],s=3)#, '-', c='k', zorder = 4)
