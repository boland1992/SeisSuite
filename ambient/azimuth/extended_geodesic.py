...
def latitude(dist, sigma01, alpha0, lon0, R=6371):
    sigma = sigma01 + dist/R
    lat = degrees(asin(cos(alpha0)*sin(sigma)))
    #alpha = atan2(tan(alpha0),cos(sigma))
    return lat
    
def longitude(dist, sigma01, alpha0, lon0,  R=6371):
    sigma = sigma01 + dist/R
...
def waypoint_init(path_info, km=km_points):
    lon1, lat1, lon2, lat2 = radians(path_info[0]), \
    ...
    lon0 = lon1 - lon01    
    npts = 1000./20#max(int((np.ceil(dist) + 1)/km), 100)

    all_d = np.linspace(-1000,0,npts)#/R 
    lons = vlon_func(all_d, sigma01, alpha0, lon0) 
    lats = vlat_func(all_d, sigma01, alpha0, lon0)   
