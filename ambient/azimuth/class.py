...
class InShape:
    ...
    def __init__(self, input_shape, coords):
...
class InPoly:
    ...
    def points_in_node_poly(self, polygon, points):
        """
        Function that takes a many (2,N) points, and uses the 
        contains() function in class matplotlib Path to check if point is
        in the polygon. 
        """
        points_in = polygon.contains_points(points)
        output = points[points_in == True]
        return np.asarray(output)
        
    def points_out_node_poly(self, polygon, points):
        """
        Function that takes a many (2,N) points, and uses the 
        contains() function in class matplotlib Path to check if point is
        oustide of the polygon. 
        """
        points_in = polygon.contains_points(points)
        output = points[points_in == False]
        return np.asarray(output)
            
    def bounds_shapefile_poly(self):
        """
        Function that returns boundaries of a shapefile polygon.
        """
        self.nodes = self.poly_nodes()
        xmin, xmax = np.min(self.nodes[:,0]), np.max(self.nodes[:,0])
        ymin, ymax = np.min(self.nodes[:,1]), np.max(self.nodes[:,1])

        return xmin, xmax, ymin, ymax
        
    def bounds_node_poly(self, nodes):
        """
        Function that returns boundaries of a shapefile polygon.
        """
...
UNIQUE_SHAPE = InShape(shape_path, coords).shape_poly()

#INNER_POINTS = np.column_stack(UNIQUE_SHAPE.buffer(5, resolution=10).interiors.coords.xy)
#INNER_SHAPE = asPolygon(UNIQUE_SHAPE.buffer(0.4, resolution=1).interiors)
#SMALL_SHAPE = asPolygon(UNIQUE_SHAPE.buffer(-0.4, resolution=1))
LARGE_SHAPE = asPolygon(UNIQUE_SHAPE.buffer(0.4, resolution=1).exterior)
large_points = np.column_stack(LARGE_SHAPE.exterior.coords.xy)
INPOLY = InPoly(shape_path, coords)

minx, maxx, miny, maxy = INPOLY.bounds_node_poly(large_points)

X = abs(maxx - minx) * np.random.rand(1e4,1) + minx
Y = abs(maxy - miny) * np.random.rand(1e4,1) + miny
many_points = np.column_stack((X,Y))

large_poly = INPOLY.node_poly(large_points)

large_poly_points = INPOLY.points_in_node_poly(large_poly, many_points)

cluster_points = kmeans(large_poly_points, 130)[0]


#convert polygonadapter class to LinearRing class
#SHAPE = asLinearRing(SHAPE)
#help(SHAPE.parallel_offset)
#SHAPE.parallel_offset(0.5)
#print type(SHAPE)
#SHAPE = asPolygon(SHAPE)
#print type(SHAPE)
#scaled_shape = scale(SHAPE, xfact=1.05, yfact=1.05)
lonmin, latmin, lonmax, latmax = InShape(shape_path, coords).shape_bounds()

SMALL_SHAPE = scale(UNIQUE_SHAPE, xfact=0.3, yfact=0.3)

small_points = np.column_stack(SMALL_SHAPE.exterior.coords.xy)
...
patch1 = PolygonPatch(LARGE_SHAPE, facecolor='blue', edgecolor='blue',
                     alpha=0.5, zorder=1)  
patch2 = PolygonPatch(UNIQUE_SHAPE, facecolor='orange', edgecolor='orange',
                   alpha=0.5, zorder=2)
PATCH = PolygonPatch(SMALL_SHAPE, facecolor='green', edgecolor='green',
                    alpha=0.5, zorder=3)
...
ax.set_title("a) xfact=1.1, yfact=1.1")
plt.xlabel('longitude (degrees)')
plt.ylabel('latitude (degrees)')
ax.set_xlim(lonmin-0.05*abs(lonmax-lonmin), lonmax+0.05*abs(lonmax-lonmin))
ax.set_ylim(latmin-0.05*abs(latmax-latmin),latmax+0.05*abs(latmax-latmin))


minx, maxx, miny, maxy = INPOLY.bounds_shapefile_poly()

X = abs(maxx - minx) * np.random.rand(1e4,1) + minx
Y = abs(maxy - miny) * np.random.rand(1e4,1) + miny
many_points = np.column_stack((X,Y))

# Generate both large and small polygons
large_poly = INPOLY.node_poly(large_points)
small_poly = INPOLY.node_poly(small_points)

# Check and return all points from many_points that ARE in large_poly
large_poly_points = INPOLY.points_in_node_poly(large_poly, many_points)

# Check and return all points from many_points that ARE outside of small_poly
small_poly_points = np.asarray(INPOLY.points_out_node_poly(small_poly, large_poly_points))

cluster_points = np.asarray(kmeans(small_poly_points, 130)[0])

plt.figure()
plt.scatter(small_poly_points[:,0], small_poly_points[:,1], s=10, alpha=0.8,c='blue')
plt.scatter(cluster_points[:,0], cluster_points[:,1], s=30,c='orange')
plt.show()
...
def rand_hull2(hull, points, K):
    "Generate K new random points contained within a convex hull"
    
    
    minx, maxx = np.min(points[:,0]), np.max(points[:,0])  
    miny, maxy = np.min(points[:,1]), np.max(points[:,1])
    X = abs(maxx - minx) * np.random.rand(100*K,1) + minx
    Y = abs(maxy - miny) * np.random.rand(100*K,1) + miny
    new_coords = np.column_stack((X,Y))
...
def poly_check(original_point):
    """ Function that takes a list of shapely.geometry.polygon.Polygon
    objects (poly_list) and checks if a single point is contained
    in this list.
    """
    #convert point coordinate to Point class    
    single_point = Point(original_point[0], original_point[1])
    #create list of True and False. If True in list, then point is in 
    #list of polygons!
    check = [polygon.contains(single_point) for polygon in POLYGONS]
    
    if True in check:
        #return True if point in set of polygons
        return original_point

    else:
        #return False if point not in set of polygons
        return [0.,0.]
...
def multipoly_check(point_set):
    return map(poly_check, point_set)
...
while infinite_counter <= 1:
    ...
    #-------------------------------------------------------------------------
    # Find coordinates of low density within path density field.
    #-------------------------------------------------------------------------

    search = np.where(H<0.1*np.average(H))#+ 0.20 * (np.max(H) - np.average(H)))
    ...
    fig20 = plt.figure()
    plt.scatter(low_density_coords[:,0], low_density_coords[:,1])
    plt.scatter(N_cluster_points[:,0], N_cluster_points[:,1], s=30, c='orange')
    fig20.savefig('low_density_points.png')
    fig20.clf()
    ...
    if grad_check1 < grad_ideal and avg_ideal < H_avg1:     
        ...
        #fig2 = plt.figure()
        #plt.pcolormesh(xedges,yedges,H_masked)
        ...
        plt.pcolor(xedges, yedges, H_masked, norm=LogNorm(vmin=np.min(H_masked), \
        vmax=np.max(H_masked)), cmap='rainbow', alpha=0.7, zorder = 3)
        plt.xlabel('longitude (degrees)')
        plt.ylabel('latitude (degrees)')
