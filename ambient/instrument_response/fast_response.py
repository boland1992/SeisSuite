...
RESP = Response(resource_id=None, instrument_sensitivity=627192000.0, 
                instrument_polynomial=None, response_stages=None)



# must specify a sampling rate 
#RESP.plot(1e-4, sampling_rate=None)
...
for xml_path in abs_paths:
    ...
    root = ET.iterparse(xml_path)
    instr_resp = [element[1] for element in root \
                         if 'InstrumentSensitivity' in element[1].tag]

    root = ET.iterparse(xml_path)

    instr_sens = [element[1] for element in root \
                      if 'Sensor' in element[1].tag]

    root = ET.iterparse(xml_path)

    instr_sample = [element[1] for element in root \
                      if 'SampleRate' in element[1].tag]
    
    instr_sample = [sample.text for sample in instr_sample]
    
    
    instr_list = np.unique([j.text for i in  instr_sens for j in i])

    resp_freqs = []
    resp_values = []
    
    for sensitivity in instr_resp:
        for resp_info in sensitivity: 
            if 'Value' in resp_info.tag:
                resp_values.append(resp_info.text)
            elif 'Frequency' in resp_info.tag:
                resp_freqs.append(resp_info.text)

    print resp_values
    print instr_sample
    print resp_freqs
    # the following array contains channel sample rate, response value and freq
    response_array = [instr_sample, resp_values[:1], resp_freqs]
    ...
    # surpress dumping pickle file for this test
