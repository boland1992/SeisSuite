...
def run_all(folder_path=None, extension='xml'):
    #folder_path =  '/storage/ANT/METADATA/METADATA'
    #extension = 'xml'
    # get files and sort by file size!
    abs_paths = paths(folder_path, extension)
    ...
    for xml_path in abs_paths:
        ...
        #xml_path = 'IRIS.CB.xml' 
        loop_t0 = datetime.datetime.now()
        
        root = ET.iterparse(xml_path)

        #top_dog = [element[1] for element in root \
        #                     if 'FDSNStationXML' in element[1].tag]
    
        #doc = iterparse(filename, ('start', 'end'))                    

    
        networks = [element[1] for element in root \
                             if 'Network' in element[1].tag]
    
    
    
        # list of stations in given network
        available_stations = []
    
        for network in networks: 
            netcode = network.get('code')
            #network_string = ET.tostring(network)

            for station in network:
                if '}Station' in station.tag:

                    statcode = station.get('code')
                    if statcode not in available_stations:
                        available_stations.append(statcode)


                    outcode = 'IRIS.{}.{}.xml'.format(netcode, statcode)
                    
                
                    with open(outcode,'wb') as f:
                        station_string = ET.tostring(station)
                        #print xml_string
        ...
        for network in networks:
            for station in network:
                if '}Station' in station.tag:
                    if station.get('code') is not statcode:
                        try:
            ...
            with open('outfile.xml','wb') as f:
                full_string = ET.tostring(network)
                print full_string
                f.write(full_string)

        loop_t1 = datetime.datetime.now()
        print 'Loop time: ', loop_t1-loop_t0
...
if __name__ == "__main__":
    ...
    args = sys.argv
    if len(sys.argv) < 2:
        raise Exception('Please input a folder path to scan for XML files!')    
    folder_path = args[1]
    run_all(folder_path=folder_path)