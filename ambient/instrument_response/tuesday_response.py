...
from obspy.station.response import Response, ResponseStage, InstrumentSensitivity
...
ttotal0 = datetime.datetime.now()

for xml_path in abs_paths[:20]:
    # test with first response file  
    loop_t0 = datetime.datetime.now()
    
    root = ET.iterparse(xml_path)

    channel_info = [element[1] for element in root \
                         if '}Channel' in element[1].tag]
    ...
    for channel in channel_info: 
        ...
        for attribute in channel: 
            if 'SampleRate' in attribute.tag: 
                sample_rate = float(attribute.text)
            if 'Response' in attribute.tag:
                

                
                for resp_info in attribute:
                    
                    if 'InstrumentSensitivity' in resp_info.tag:
                
                        for info in resp_info:
                            if 'Value' in info.tag:
                                resp_value = float(info.text)
                            elif 'Frequency' in info.tag:
                                resp_freq = float(info.text)
                            elif 'InputUnits' in info.tag:
                                for name in info:
                                    if 'Name' in name.tag:
                                        input_units = name.text
                            elif 'OutputUnits' in info.tag:
                                for name in info:
                                    if 'Name' in name.tag:
                                        output_units = name.text                                
                                
                    instr_sens = InstrumentSensitivity(resp_value, 
                                                       resp_freq, 
                                                       input_units,
                                                       output_units)
                                
                    if 'Stage' in resp_info.tag:
                        #print resp_info.get('number')
                        stage_sequence_number = int(resp_info.get('number'))

                        for info in resp_info:
                            if 'StageGain' in info.tag:
                                for attrib in info:
                                    if 'Value' in attrib.tag:
                                        stage_gain = float(attrib.text)

                                    if 'Frequency' in attrib.tag:
                                        stage_gain_frequency = float(attrib.text)
                            
                            if 'PolesZeros' in info.tag:
                                for attrib in info:
                                    if 'InputUnits' in attrib.tag:
                                        for name in attrib:
                                            if 'Name' in name.tag:
                                                input_units = name.text
                                    if 'OutputUnits' in attrib.tag:
                                        for name in attrib:
                                            if 'Name' in name.tag:
                                                output_units = name.text                                                
                                                
                        stage = ResponseStage(stage_sequence_number, 
                                              stage_gain,
        ...
        data = [sample_rate, resp_value, resp_freq]
        RESP = Response(resource_id=None, instrument_sensitivity=instr_sens, 
                instrument_polynomial=None, response_stages=stages_list)
        
        min_freq = 1e-4
        t_samp = 1.0 / sample_rate
        nfft = sample_rate / min_freq
        RESP.get_evalresp_response(sample_rate, nfft, output="VEL",
                                   start_stage=None, end_stage=None)

        RESP.plot(min_freq, sampling_rate=sample_rate)
        total_resp.append(data)
...
total_resp = np.asarray(total_resp)

ttotal1 = datetime.datetime.now()

print "Total loops processing time was:", ttotal1-ttotal0
